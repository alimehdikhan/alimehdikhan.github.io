...
export function Hero() {
  ...
  useEffect(() => {
    ...
    const particleCount = isMobile
      ? 8
      : Math.min(Math.max(Math.floor((window.innerWidth * window.innerHeight) / 45000), 10), 28);
      ...
      <div className="container mx-auto px-4 sm:px-6 relative z-10 w-full">
        ...
        >
          <div className="md:col-span-7 flex flex-col gap-6">
            ...
            <motion.div className="text-xl sm:text-2xl text-text-secondary font-medium flex items-center min-h-8" variants={itemVariants}>
              <span>I&apos;m a&nbsp;</span>
            ...
            <motion.div className="flex flex-wrap gap-3 text-sm text-text-tertiary" variants={itemVariants}>
              {credibilityChips.map((chip) => (
                <span key={chip} className="flex items-center gap-1.5">
                  <svg className="w-4 h-4 text-indigo-400 shrink-0" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24" aria-hidden="true">
            ...
            <motion.div className="flex gap-4 pt-4" variants={itemVariants}>
              {[
                ...
                >
                  <svg width="18" height="18" fill={social.stroke ? 'none' : 'currentColor'} stroke={social.stroke ? 'currentColor' : 'none'} strokeWidth={social.stroke ? '2' : '0'} viewBox="0 0 24 24" aria-hidden="true">
                    <path d={social.icon} />
                  </svg>
                </motion.a>
              ))}
